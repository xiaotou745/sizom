$(document).ready(function () {
    new AjaxUpload($("#imageupload"), {
        ...
        action: '/adminaccount/TopicImageUpload',
        responseType: 'Json',
    ...
    function bindclick() {
        ...
        $(".imagedelete").unbind("click").click(function () {
            ...
            $.ajax({
                url: '/adminAccount/TopicImageDelete',
                type: 'POST',
                dataType: 'Json',
                data: { Id: index },
    ...
    iniselect($(".topictype"), $("#TopicType"));
    $("#topicsubmit").click(function() {
        if (!$.formValidator.pageIsValid('1')) {
            return;
        }
        $.ajax({
            url: '/adminAccount/TopicCreateSave',
            type: 'POST',
            dataType: 'Json',
            ...
            success: function(returndata) {
                if (!returndata.IsError) {
                    alert("保存成功")
                    window.location.href = "/adminaccount/topics";
                } else {
                    alert(returndata.Message);
                }
            },
